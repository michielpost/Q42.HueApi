...
namespace HueApi.ColorConverters.Original
{
  ...
  internal static partial class HueColorConverter
  {
    ...
    /// <returns></returns>
    public static string HexFromState(Light state)
    {
      if (state == null)
        throw new ArgumentNullException(nameof(state));
      if (state.On.IsOn == false || state.Dimming?.Brightness <= 5) //Off or low brightness
        return "000000";
      if (state.Color?.Xy != null) //Based on XY value
        return HexFromXy(state.Color.Xy.X, state.Color.Xy.Y);
    ...
    *****************************************************************************************/
    ...
    /// <param name="state"></param>
    /// <returns></returns>
    public static string HexColorFromState(Light state, string model)
    {
      if (state == null)
        throw new ArgumentNullException(nameof(state));
      if (state.On.IsOn == false || state.Dimming?.Brightness <= 1) //Off or low brightness
        return "000000";
      if (state.Color?.Xy != null) //Based on XY value
      {
        RGBColor color = ColorFromXY(new CGPoint(state.Color.Xy.X, state.Color.Xy.Y), model);

        //Brightness of state (0.0-1.0)
        double b = Convert.ToDouble(state.Dimming?.Brightness) / 255;
    ...
    /// <param name="state"></param>
    /// <returns></returns>
    public static RGBColor RGBColorFromState(Light state, string model)
    {
      if (state == null)
        throw new ArgumentNullException(nameof(state));
      ...
      if (state.Color?.Xy != null) //Based on XY value
      {
        return ColorFromXY(new CGPoint(state.Color.Xy.X, state.Color.Xy.Y), model);
      }
