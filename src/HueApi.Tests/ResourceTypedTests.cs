...
{
  [TestClass]
  public class ResourceTypedTests
  {
    private readonly LocalHueApi localHueClient;

    public ResourceTypedTests()
    {
    ...
    [TestMethod]
    public async Task GetLightTest()
    {
      var result = await localHueClient.GetResourceTypedAsync("light");
    ...
    public async Task LoopAllTest()
    {
      ...
      {
        var resById = await localHueClient.GetResourceTypedAsync(res);
    ...
    public async Task LoopAllClipTest()
    {
      ...
      {
        var resByType = await localHueClient.GetResourceTypedAsync(rtype);
